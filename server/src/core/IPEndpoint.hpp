...
struct Hasher
{
    std::size_t operator()(const IPEndpoint& endpoint) const noexcept
    {
        std::size_t h1 = std::hash<uint32_t>{}(endpoint.m_sa.sin_addr.s_addr);
        std::size_t h2 = std::hash<uint16_t>{}(endpoint.m_sa.sin_port);
